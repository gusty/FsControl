...
let inline fmap   f x = Inline.instance (Map, x, x) f
...
// add ìnstance for Functor class
    static member instance (_Functor:Map, x:Tree<_>, _, _) = fun f -> Tree.map f x
...
type Product<'a> = Product of 'a with
    static member inline instance (Mempty, _:Product<'n>      ) = fun ()              -> Product 1G     :Product<'n>
