...
type State<'S,'A> with
    static member instance (Functor.Map  , State m,                _, _) = fun f -> State(fun s -> let (a, s') = m s in (f a, s'))       :State<'s,_>
    ...
    static member instance (Applicative.Apply, f:State<'s,_>, x:State<'s,'a>, _, _:State<'s,'b>) = fun () -> DefaultImpl.ApplyFromMonad f x :State<'s,'b>