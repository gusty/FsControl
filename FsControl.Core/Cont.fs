...
type Cont<'R,'A> with
    static member instance (Functor.Map, Cont m:Cont<'r,'a>, _, _) = fun (f:_->'b) -> Cont(fun c -> m (c << f))
    ...
    static member instance (Applicative.Apply, f:Cont<'r,_>, x:Cont<'r,'a>, _, _:Cont<'r,'b>) = fun () -> DefaultImpl.ApplyFromMonad f x :Cont<'r,'b>