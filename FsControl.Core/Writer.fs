...
type Writer<'W,'A> with
    static member        instance (Functor.Map,   Writer(a,w),                  _ ,_) = fun f -> Writer(f a, w) :Writer<'w,_>
    ...
    static member inline instance (Applicative.Apply, f:Writer<'w,_>, x:Writer<'w,'a>, _, _:Writer<'w,'b>) = fun () -> DefaultImpl.ApplyFromMonad f x :Writer<'w,'b>